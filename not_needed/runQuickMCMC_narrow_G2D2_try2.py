...
par_tdb = Path("/scratch/na00078/projects/IPTA_MDC2/mdc2/group2/dataset_2/par_tdb")
par_orig = Path("/scratch/na00078/projects/IPTA_MDC2/mdc2/group2/dataset_2/par")
...
# 1) PINT first (tdb)
for par in sorted(par_tdb.glob("*_tdb.par")):
    name = par.stem.replace("_tdb","")
    tim  = timdir / f"{name}.tim"
    if not tim.exists():
        print(f"❌ missing TIM for {name}")
        continue
    try:
        p = Pulsar(str(par), str(tim), ephem="DE436", timing_package="pint")
        if len(p.toas) > 0:
            psrs.append(p); loaded.add(name)
            print(f"✅ (PINT) {name}: {len(p.toas)} TOAs")
        else:
            print(f"⚠️ (PINT) {name}: 0 TOAs, skipped")
    except Exception as e:
        print(f"❌ (PINT) {name}: {e}")

# 2) tempo2 fallback for the rest (original TCB/T2)
for par in sorted(par_orig.glob("*.par")):
    name = par.stem
    if name in loaded:
        continue
    tim = timdir / f"{name}.tim"
    if not tim.exists():
        ...
        continue
    try:
        p = Pulsar(str(par), str(tim), ephem="DE436", timing_package="tempo2")
        if len(p.toas) > 0:
            psrs.append(p)
            print(f"✅ (T2) {name}: {len(p.toas)} TOAs")
        else:
            print(f"⚠️ (T2) {name}: 0 TOAs, skipped")
    except Exception as e:
        print(f"❌ (T2) {name}: {e}")

print(f"\nLoaded {len(psrs)} pulsars (PINT+tempo2)")
