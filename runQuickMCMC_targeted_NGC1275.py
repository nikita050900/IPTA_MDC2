...
import QuickCW.QuickCW_targeted_NGC1275 as QuickCW
...
noisefile = '/scratch/js0311/15yr_data/15yr_v1_1/v1p1_wn_dict.json'
...
savefile = '/scratch/js0311/jet_source_project_results/targeted_freq_upperlimit_runs/NGC1275_targeted_671E8_wJ1713_N1e9_thin10.h5'
#savefile = None

###############
#targeted search params-LondonAdd
c = SkyCoord(ra = '03h19m48.1601s', dec = '41d30m42.106s')
...
chain_params = ChainParams(T_max,n_chain, n_block_status_update,
                           freq_bounds=np.array([TargFreq-.5e-13, TargFreq+.5e-13]), #prior bounds used on the GW frequency (a lower bound of np.nan is interpreted as 1/T_obs)
...
pta,mcc = QuickCW.QuickCW(chain_params, psrs,
                                  amplitude_prior='UL', #specify amplitude prior to use - 'detection':uniform in log-amplitude, 'UL': uniform in amplitude
